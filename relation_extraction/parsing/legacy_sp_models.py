...
from RECON.utils import embedding_utils, graph, context_utils
from RECON.semanticgraph import graph_utils
from RECON.utils.conversion_util import calculate_order_conversion
...
MAX_NUM_NODES = 9
def set_max_edges(max_edges_per_graph, max_num_nodes):
  global MAX_EDGES_PER_GRAPH
...
def to_indices_with_real_entities_and_entity_nums_with_vertex_padding_ca(graphs, word2idx, property2idx, max_sent_len, mode='train', **kwargs):
    """
    :param graphs:
    :param word2idx:
    :param property2idx:
    ...
    for index, g in enumerate(tqdm.tqdm(graphs, ascii=True)):
        ...
        for j, edge in enumerate(g["edgeSet"][:MAX_EDGES_PER_GRAPH]):
            new_j = calculate_order_conversion(j, len(g["vertexSet"]), PAD_ENT_CNT=MAX_NUM_NODES)
...
def to_indices_with_real_entities_and_entity_nums_with_vertex_padding(graphs, word2idx, property2idx, max_sent_len, mode='train', **kwargs):
    ...
    y_matrix = np.zeros((len(graphs), MAX_EDGES_PER_GRAPH), dtype="int16")
    entity_cnt = []
    entity_indices = -1*np.ones((len(graphs), MAX_EDGES_PER_GRAPH, 2), dtype="int32")
    entity_surface_forms = np.empty((len(graphs), MAX_EDGES_PER_GRAPH, 2), dtype=object)
    ...
    for index, g in enumerate(tqdm.tqdm(graphs, ascii=True)):
        ...
        for j, edge in enumerate(g["edgeSet"][:MAX_EDGES_PER_GRAPH]):
            ...
            property_kbid = property2idx.get(property_kbid, property2idx[embedding_utils.unknown])
            y_matrix[index, new_j] = property_kbid
            entity_indices[index, new_j, 0] = context_utils.get_entityIdx_from_tokens(g['tokens'], edge['left'], vertex2qids, kwargs['entity2idx'])
            entity_indices[index, new_j, 1] = context_utils.get_entityIdx_from_tokens(g['tokens'], edge['right'], vertex2qids, kwargs['entity2idx'])
            entity_surface_forms[index, new_j, 0] = [g['tokens'][ti] for ti in edge['left']]
            entity_surface_forms[index, new_j, 1] = [g['tokens'][ti] for ti in edge['right']]

            entity_pair_instance.append((pos2id[tuple(edge['left'])], pos2id[tuple(edge['right'])]))
            token_positions_instance.append((g['tokens'][edge['left'][0]:edge['left'][-1] + 1],
                g['tokens'][edge['right'][0]:edge['right'][-1] + 1]))
        entity_pair.append(entity_pair_instance)
        token_positions.append(token_positions_instance)

    entity_cnt = np.array(entity_cnt, dtype=np.int32)        
         
    return sentences_matrix, entity_matrix, y_matrix, entity_cnt, entity_indices, entity_surface_forms, entity_pair, token_positions

def to_indices_with_real_entities_and_entity_nums_with_vertex_padding_and_negative_sampling(graphs, word2idx, property2idx, max_sent_len, idx2property, mode='train', **kwargs):
    """
    :param graphs:
    :param word2idx:
    :param property2idx:
